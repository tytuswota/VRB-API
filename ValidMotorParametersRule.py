class ValidMotorParametersRule():
    ...
    def validateMotorIDS(self, parameters):
        if parameters[1] in self.VALID_MOTOR_IDS:
            return True
        return False
    
    def validateMotorActions(self, parameters):
        if parameters[2] in self.VALID_MOTOR_ACTIONS:
            return True
        return False
    
    def validFractionalPower(self, parameters):
        if self.FRACTIONAL_POWER_LIMITS[0] <= parameters[3] <= self.FRACTIONAL_POWER_LIMITS[1]:
            return True
        return False
